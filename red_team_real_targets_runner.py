...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'red_team_campaign_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class RedTeamCampaignExecutor:
    ...
    def __init__(self, targets_file: str = 'campaign_targets.json'):
    ...
    def _execute_single_campaign_attempt(self, campaign_type: str, target_set: str, 
        ...
        campaign_path = Path('campaigns') / campaign_type
    ...
    def save_comprehensive_report(self, filename: Optional[str] = None) -> str:
        ...
        if not filename:
            filename = f"red_team_campaign_execution_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        logger.info(f"Comprehensive execution report saved to {filename}")
        return filename
